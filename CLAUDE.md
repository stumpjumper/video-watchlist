...
- Certs in `certs/` (gitignored); renew with `~/bin/renew_tailscale_https_cert`. Auto-renews twice monthly via launchd (`com.tailscale-cert-renew`, 1st/15th 4am) and emails success/failure to Alfred via the Gmail API through a dedicated OneCLI agent (`videowatchlist`) — see `scripts/notify_cert_status.mjs`. 90-day cert lifetime is a Let's Encrypt/Tailscale hard limit, not configurable.
...
- A same-origin navigation to a `Content-Disposition: attachment` response can strand the user outside the SPA on iOS, especially in the installed home-screen PWA (`WKWebView` has no reliable TLS/nav-error override UI either — same underlying limitation). Prefer building downloads client-side from data already in memory via a Blob + temporary `<a download>` click, never a real page navigation.
